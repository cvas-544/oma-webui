	...
	<div slot="content">
		<DropdownMenu className="w-70 max-h-72 overflow-hidden transition">
			{#if tab === ''}
				...
				>
					<!-- OMA: v0.11.0 strips Upload Files / Capture / Attach Webpage / Attach Notes /
					     Attach Knowledge / Reference Chats / Google Drive / OneDrive. Only two artifact
					     pickers remain. Original items kept below in {#if false} for easy re-enable. -->

					<!-- OMA: Attach Image Artifacts — opens image-filtered artifact picker -->
					<button
						class="flex gap-2 w-full items-center h-[1.6875rem] px-2 text-[13px] font-normal select-none cursor-pointer hover:bg-gray-50/40 dark:hover:bg-gray-800/40 rounded-xl"
						on:click={() => {
							tab = 'files-images';
						}}
					>
						...
						>
							<path
								fill-rule="evenodd"
								d="M2 4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V4Zm2-.5a.5.5 0 0 0-.5.5v5.379l1.544-1.548a.75.75 0 0 1 1.06-.003l2.158 2.147 1.492-1.492a.75.75 0 0 1 1.06 0L11.5 10.12V4a.5.5 0 0 0-.5-.5H4ZM3.5 12v-.836l2.07-2.075 2.158 2.147a.75.75 0 0 0 1.06-.003l1.492-1.492 1.72 1.72V12a.5.5 0 0 1-.5.5H4a.5.5 0 0 1-.5-.5Zm5.25-6.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Z"
								clip-rule="evenodd"
							/>
						</svg>
					...
					<!-- OMA: Attach File Artifacts — opens document-filtered artifact picker -->
					<button
						class="flex gap-2 w-full items-center h-[1.6875rem] px-2 text-[13px] font-normal select-none cursor-pointer hover:bg-gray-50/40 dark:hover:bg-gray-800/40 rounded-xl"
						on:click={() => {
							tab = 'files-docs';
					...
					{#if false}
						<!-- OMA: original menu items preserved for re-enable -->
						<Tooltip
			...
			{:else if tab === 'files-images'}
				<div class="flex max-h-72 flex-col overflow-hidden" in:fly={{ x: 20, duration: 150 }}>
					...
					>
						...
						<div class="flex items-center w-full justify-between">
							<div>
								{$i18n.t('Image Artifacts')}
							</div>
						</div>
					</button>

					<Files filter="image" {onSelect} />
				</div>
			{:else if tab === 'files-docs'}
				<div class="flex max-h-72 flex-col overflow-hidden" in:fly={{ x: 20, duration: 150 }}>
					<button
						class="flex w-full shrink-0 justify-between gap-2 items-center h-[1.6875rem] px-2 text-[13px] font-normal select-none cursor-pointer rounded-xl hover:bg-gray-50/40 dark:hover:bg-gray-800/40"
						on:click={() => {
					...
					>
						...
						<div class="flex items-center w-full justify-between">
							<div>
								{$i18n.t('File Artifacts')}
							</div>
						</div>
					</button>

					<Files filter="file" {onSelect} />
				</div>
			{:else if tab === 'chats'}
				<div class="flex max-h-72 flex-col overflow-hidden" in:fly={{ x: 20, duration: 150 }}>
					<button
