<script lang="ts">
	...
	export let onSubmit: (data: { rating: string; feedback: string; reasons: string[] }) => void = () => {};
	...
	function toggleReason(pill: string) {
		selectedReasons = selectedReasons.includes(pill)
			? selectedReasons.filter((r) => r !== pill)
			: [...selectedReasons, pill];
	}
	...
	async function handleSubmit() {
		...
		submitting = true;
		try {
			await onSubmit({ rating: selected, feedback, reasons: selectedReasons });
		} finally {
			submitting = false;
			resetAndClose();
		}
			...
			{#if selected}
				...
				>
					...
					<div class="flex flex-wrap gap-1.5 mb-3">
						{#each PILLS as pill}
							{@const active = selectedReasons.includes(pill)}
							<button
								type="button"
								on:click={() => toggleReason(pill)}
								class="rounded-full px-3 py-1 text-[11px] font-medium border transition-colors leading-none
									{active
										? 'bg-[#003877] text-white border-[#003877]'
										: 'bg-white text-zinc-500 border-zinc-200 hover:border-[#73B2F2] hover:text-[#003877]'}"
							>
								{pill}
							</button>
						{/each}
					...
					<textarea
						...
						rows="2"
						class="w-full resize-none rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-[13px] text-zinc-800 leading-relaxed placeholder:text-zinc-400 focus:border-[#73B2F2] focus:outline-none transition-colors"
					/>
					...
					<div class="mt-3 flex items-center justify-between border-t border-zinc-100 pt-3">
						...
						>
							{#if submitting}
								<div class="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin mx-auto"></div>
							{:else}
								Send
							{/if}
						</button>
