<script lang="ts">
	...
	$: {
		...
		const chartRe = /!\[[^\]]*\]\((https?:\/\/[^)]+)\)/g;
		while ((m = chartRe.exec(content)) !== null) items.push({ name: 'chart.png', url: m[1] });
	...
	// Fetch-as-blob so cross-origin URLs actually download instead of opening in-tab.
