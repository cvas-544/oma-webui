<script lang="ts">
	...
	function runningTitle(steps: { type: string; tool: string }[]): string {
		...
		return steps.length === 0 ? 'Thinking...' : 'Working on it';
	}
			...
			<div class="flex items-center gap-2 min-w-0">
				...
				{#if steps.length > 0}
					<span class="text-[11px] text-gray-400 dark:text-gray-500 ml-1">
						{streaming ? `${steps.length} ${steps.length === 1 ? 'step' : 'steps'}…` : `${steps.length} ${steps.length === 1 ? 'step' : 'steps'}`}
					</span>
				{/if}

				{#if streaming}
					{#if steps.length === 0}
						<svg class="w-3.5 h-3.5 shrink-0 animate-spin" style="color: #73B2F2;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
