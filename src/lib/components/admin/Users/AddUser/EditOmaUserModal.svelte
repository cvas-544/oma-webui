<script lang="ts">
	// ---------------------------------------------------------------------------
	// File:        EditOmaUserModal.svelte
	// Description: OMA-specific Edit User modal. Same 3-tab layout as AddOmaUserModal
	//              (General / Permissions / Preview), pre-populated from selectedUser.
	//              No password field — users authenticate via Azure AD SSO.
	// Author:      Vasu Chukka
	// ---------------------------------------------------------------------------
	import { toast } from 'svelte-sonner';
	import { createEventDispatcher, getContext } from 'svelte';
	import { updateUserById, deleteUserById } from '$lib/apis/users';
	...
	export let sessionUser: any = null;

	// OMA static data passed in from UserList
	...
	const init = () => {
		...
		// TODO: load OMA RBAC fields from GET /api/v1/oma/users/{email}
		// For now initialize to defaults; wire once OMA backend endpoint exists
		rbacRole = selectedUser.rbac_role ?? 'betriebsfuehrer';
		kuerzel = selectedUser.kuerzel ?? '';
		selectedPlantIds = selectedUser.plant_ids ?? [];
		selectedModelIds = selectedUser.models ?? [];
		selectedKbIds = selectedUser.knowledge_bases ?? [];
		selectedTab = 'general';
	};
	...
	const submitHandler = async () => {
		if (!name.trim() || !email.trim()) {
			toast.error($i18n.t('Name and email are required.'));
			selectedTab = 'general';
			return;
		}
		...
		if (res) {
			// TODO: update OMA RBAC record: PUT /api/v1/oma/users/{email}
			// { rbac_role, kuerzel, plant_ids, models, knowledge_bases }
			toast.success($i18n.t('User updated.'));
			dispatch('save');
			show = false;
		}
	...
	const deleteHandler = async () => {
		...
		if (selectedUser.id === sessionUser?.id) {
			toast.error($i18n.t('You cannot delete your own account.'));
			return;
		}

		loading = true;

		const res = await deleteUserById(localStorage.token, selectedUser.id).catch((err) => {
			toast.error(`${err}`);
			return null;
		});
